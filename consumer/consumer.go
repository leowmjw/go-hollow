...
func (c *Consumer) findNearestSnapshot(fromVersion, toVersion int64) *blob.Blob {
	// Look for snapshots between fromVersion and toVersion
	for version := toVersion; version > fromVersion; version-- {
		if snapshot := c.retriever.RetrieveSnapshotBlob(version); snapshot != nil {
			return snapshot
		}
	}
	return nil
